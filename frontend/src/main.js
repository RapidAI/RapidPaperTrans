...
// 模式切换时保存的 PDF 路径
let latexModeLeftPdf = null;  // LaTeX 模式左侧 PDF 路径
let latexModeRightPdf = null; // LaTeX 模式右侧 PDF 路径
let pdfModeLeftPdf = null;    // PDF 模式左侧 PDF 路径
let pdfModeRightPdf = null;   // PDF 模式右侧 PDF 路径
...
async function downloadFile(type) {
    ...
    if (!currentResult) {
        showToast('没有可下载的内容，请先处理文档', 'warning');
        return;
    }
    ...
    try {
        ...
        switch (type) {
            case 'chinese':
                showToast('正在准备中文 PDF...', 'info');
                savePath = await DownloadChinesePDF();
                if (savePath) {
                    showToast('中文 PDF 已保存', 'success');
                }
                break;
            case 'bilingual':
                showToast('正在生成中英对照 PDF...', 'info');
                savePath = await DownloadBilingualPDF();
                if (savePath) {
                    showToast('中英对照 PDF 已保存', 'success');
                }
                break;
            case 'latex':
                showToast('正在打包 LaTeX 文件...', 'info');
                savePath = await DownloadLatexZip();
                if (savePath) {
                    showToast('LaTeX 文件已保存', 'success');
                }
                break;
        }
    } catch (error) {
...
async function handleProcess() {
    ...
    if (!input) {
        showError('请输入 arXiv URL、arXiv ID 或本地 zip 文件路径');
        return;
    }
    ...
    try {
        ...
        if (existingInfo && existingInfo.exists) {
            ...
            if (existingInfo.is_complete && !userChoice) {
                ...
                if (existingInfo.paper_info && existingInfo.paper_info.arxiv_id) {
                    ...
                    if (result) {
                        ...
                        showToast('已加载现有翻译结果', 'success');
                    }
                }
                return;
            }

            // For failed translations (not can_continue), if user chooses "放弃", just return
            ...
            try {
                ...
                stopStatusPolling();
                const errorMsg = forceError.message || forceError.toString() || '处理失败';
    ...
    try {
        ...
        const errorMsg = error.message || error.toString() || '处理失败';
        updateStatus('error', 0, errorMsg);
        showError(errorMsg);
    } finally {
        setProcessingState(false);
...
async function handleProcessResult(result) {
    if (result) {
        ...
        updateStatus('complete', 100, '处理完成');
...
function updateStatus(phase, progress, message, error) {
    // 保存状态到当前模式的状态变量
    const statusState = { phase, progress, message: message || '就绪', error: error || null };
    ...
    if (error) {
        ...
        statusMessage.textContent = message || '就绪';
 ...
 * 直接应用状态显示（不保存到状态变量）
 * 用于模式切换时恢复状态
 * @param {Object} status - 状态对象 { phase, progress, message, error }
 */
...
function createPaperItem(paper) {
    ...
    item.innerHTML = `
        <span class="paper-icon">${isComplete ? '📄' : (isError ? '❌' : '⏳')}</span>
        <div class="paper-info">
            ...
            ${paper.error_message ? `<div class="paper-error" title="${escapeHtml(paper.error_message)}">错误: ${escapeHtml(paper.error_message.substring(0, 50))}${paper.error_message.length > 50 ? '...' : ''}</div>` : ''}
        </div>
        <div class="paper-actions">
            ${showView ? '<button class="paper-btn paper-btn-view" title="查看">👁️ 查看</button>' : ''}
            ${showShare ? '<button class="paper-btn paper-btn-share" title="分享到 GitHub">📤 分享</button>' : ''}
            ${showContinue ? '<button class="paper-btn paper-btn-continue" title="继续翻译">▶️ 继续</button>' : ''}
            <button class="paper-btn paper-btn-retranslate" title="重新翻译">🔄 重译</button>
            <button class="paper-btn paper-btn-delete" title="删除">🗑️</button>
        </div>
    `;
...
async function viewPaper(arxivId) {
    try {
        ...
        if (result) {
            ...
            showToast('论文加载完成', 'success');
        }
    } catch (error) {
        console.error('Failed to view paper:', error);
        showToast('加载论文失败: ' + (error.message || error), 'error');
    }
}
...
async function sharePaper(arxivId) {
    try {
        ...
        showToast('检查分享状态失败: ' + (error.message || error), 'error');
    }
}
...
async function retranslatePaper(arxivId) {
    // 检查是否正在翻译中
    if (isProcessing) {
        showToast('翻译进行中，请等待当前翻译完成后再重译', 'warning');
        return;
    }
    ...
    try {
        ...
        if (result) {
            currentResult = result;
            updateStatus('complete', 100, '重新翻译完成');
            downloadDropdown.style.display = 'inline-block';
            btnShare.style.display = 'inline-block';
        ...
        stopStatusPolling();
        updateStatus('error', 0, error.message || '重新翻译失败');
        showToast('重新翻译失败: ' + (error.message || error), 'error');
    } finally {
        setProcessingState(false);
    }
}
...
async function continuePaper(arxivId) {
    // 检查是否正在翻译中
    if (isProcessing) {
        showToast('翻译进行中，请等待当前翻译完成后再继续', 'warning');
        return;
    }
    ...
    try {
        ...
        if (result) {
            currentResult = result;
            updateStatus('complete', 100, '翻译完成');
            downloadDropdown.style.display = 'inline-block';
            btnShare.style.display = 'inline-block';
        ...
        stopStatusPolling();
        updateStatus('error', 0, error.message || '继续翻译失败');
        showToast('继续翻译失败: ' + (error.message || error), 'error');
    } finally {
        setProcessingState(false);
    }
}
    ...
    try {
        ...
        if (result.success) {
            ...
            if (result.chinese_pdf_url) {
                message += '\n中文 PDF: ' + result.chinese_pdf_url;
            }
            if (result.bilingual_pdf_url) {
                message += '\n双语 PDF: ' + result.bilingual_pdf_url;
            }
        ...
        showToast('分享失败: ' + (error.message || error), 'error');
    } finally {
        shareProgress.style.display = 'none';
        btnShareConfirm.disabled = false;
    }
...
async function updateGitHubToken() {
    githubUpdateStatus.textContent = '⏳ 正在获取 Token...';
    ...
    try {
        ...
        githubUpdateStatus.textContent = '✅ Token 已更新';
        githubUpdateStatus.className = 'test-status success';
        showToast('GitHub Token 已成功更新', 'success');
        ...
        githubUpdateStatus.textContent = '❌ 更新失败';
        githubUpdateStatus.className = 'test-status error';
        showToast('更新 Token 失败: ' + (error.message || error), 'error');
    } finally {
...
async function testGitHubConnection() {
    ...
    if (!token) {
        githubTestStatus.textContent = '❌ 请输入 GitHub Token';
        githubTestStatus.className = 'test-status error';
        return;
    }

    githubTestStatus.textContent = '⏳ 正在测试...';
    ...
    try {
        ...
        githubTestStatus.textContent = '✅ 连接成功';
        githubTestStatus.className = 'test-status success';
        showToast('GitHub 连接测试成功', 'success');
    } catch (error) {
        console.error('GitHub test failed:', error);
        githubTestStatus.textContent = '❌ ' + (error.message || '连接失败');
        githubTestStatus.className = 'test-status error';
        showToast('GitHub 测试失败: ' + (error.message || error), 'error');
    } finally {
...
async function performStartupCheck() {
    ...
    try {
        ...
        // Show error state
        updateLatexCheckResult(false, '检测失败: ' + (error.message || error));
        updateLlmCheckResult(false, '检测失败');
    }
}
...
function updateLatexCheckResult(installed, version) {
    ...
    if (installed) {
        ...
        latexDetail.textContent = version || '已安装';
        checkLatexItem.classList.add('success');
        checkLatexItem.classList.remove('error');
        latexAction.style.display = 'none';
    } else {
        latexStatus.textContent = '❌';
        latexDetail.textContent = '未检测到 LaTeX 编译器';
...
function updateLlmCheckResult(configured, error) {
    ...
    if (configured) {
        ...
        llmDetail.textContent = 'LLM 配置正确，连接测试通过';
        checkLlmItem.classList.add('success');
        checkLlmItem.classList.remove('error');
        llmAction.style.display = 'none';
    } else {
        llmStatus.textContent = '❌';
        llmDetail.textContent = error || 'LLM 未配置或连接失败';
...
function resetTestStatus() {
    ...
    if (llmChanged) {
        apiTestPassed = false;
        testStatus.textContent = '⚠️ 配置已修改，请重新测试';
...
async function testConnection() {
    ...
    if (!baseUrl) {
        testStatus.textContent = '❌ 请输入 API Base URL';
        testStatus.className = 'test-status error';
        return;
    }
    if (!model) {
        testStatus.textContent = '❌ 请输入模型名称';
        testStatus.className = 'test-status error';
        return;
    }

    // Show testing status
    testStatus.textContent = '⏳ 正在测试 LLM 连接...';
    testStatus.className = 'test-status testing';
    btnTestConnection.disabled = true;

    try {
        await TestAPIConnection(apiKey, baseUrl, model);
        testStatus.textContent = '✅ LLM 测试成功';
        testStatus.className = 'test-status success';
        apiTestPassed = true;
        showToast('LLM 连接测试成功', 'success');
    } catch (error) {
        console.error('API test failed:', error);
        const errorMsg = error.message || error.toString() || '连接失败';
        testStatus.textContent = '❌ ' + errorMsg;
        testStatus.className = 'test-status error';
        apiTestPassed = false;
        showToast('LLM 测试失败: ' + errorMsg, 'error', 5000);
    } finally {
...
async function saveSettings() {
    ...
    if (llmChanged && !apiTestPassed) {
        showToast('LLM 配置已修改，请先测试连接', 'warning');
        return;
    }

    // If no API key at all, warn user
    if (!apiKey && !originalLlmSettings.apiKey) {
        showToast('请配置 LLM API Key', 'warning');
        return;
    }
    ...
    try {
        ...
        showToast('保存设置失败: ' + (error.message || error), 'error');
    }
}
...
async function init() {
    ...
    // Set initial state
    updateStatus('idle', 0, '就绪');

    console.log('LaTeX 翻译器前端已初始化');
}
...
async function performStartupCheckOnInit() {
    try {
        ...
        showStartupCheckModal();
        updateLatexCheckResult(false, '检测失败');
        updateLlmCheckResult(false, '检测失败');
    }
}
    ...
    EventsOn('pdf-page-translated', (data) => {
        ...
        if (data && data.outputPath) {
            ...
            if (currentMode === 'pdf') {
                ...
                const message = `已翻译 ${data.currentPage}/${data.totalPages} 页`;
...
function switchMode(mode) {
    ...
    // 切换前保存当前模式的 PDF 路径（只保存有效的 PDF 路径）
    if (currentMode === 'latex') {
        // 保存 LaTeX 模式的 PDF 路径
...
async function handleSelectPdf() {
    try {
        ...
        showError('选择 PDF 文件失败: ' + (error.message || error));
    }
}
...
async function handlePdfDrop(e) {
    ...
    showToast('拖拽功能暂不支持，请使用"选择 PDF 文件"按钮', 'warning');
}
...
async function loadPdfFile(filePath) {
    try {
        updateStatus('loading', 0, '正在加载 PDF 文件...');
        ...
        if (!pdfInfo.is_text_pdf) {
            showError('该 PDF 为扫描件，不支持直接翻译');
            updateStatus('error', 0, '不支持的 PDF 格式');
            return;
        }
        ...
        updateStatus('idle', 0, 'PDF 文件已加载，可以开始翻译');
        showToast(`已加载: ${pdfInfo.file_name} (${pdfInfo.page_count} 页)`, 'success');

    } catch (error) {
        console.error('Error loading PDF:', error);
        showError('加载 PDF 失败: ' + (error.message || error));
        updateStatus('error', 0, '加载失败');
    }
}
...
function showSelectedPdfFile(pdfInfo) {
    ...
    pdfFileSize.textContent = formatFileSize(pdfInfo.file_size) + ` · ${pdfInfo.page_count} 页`;
...
function clearSelectedPdf() {
    ...
    currentPdfTranslatedPath = null; // 清除翻译后的 PDF 路径
...
async function handleTranslatePdf() {
    ...
    updateStatus('translating', 0, '开始翻译...');
    ...
    try {
        ...
        if (result) {
            ...
            showToast(`翻译完成！共翻译 ${result.translated_blocks} 个文本块`, 'success');
        }
    } catch (error) {
        console.error('PDF translation error:', error);
        stopPdfStatusPolling();

        const errorMsg = error.message || error.toString() || '翻译失败';
        updateStatus('error', 0, errorMsg);
        showError(errorMsg);
    } finally {
...
function updatePdfStatus(status) {
    ...
    // (page progress messages contain "页")
    if (status.total_blocks > 0 && !message.includes('页')) {
        message = `${message} (${status.completed_blocks}/${status.total_blocks} 块)`;
    }
...
function setPdfTranslatingState(translating) {
    ...
    if (btnTranslatePdf) {
        ...
        btnTranslatePdf.textContent = translating ? '⏳ 翻译中...' : '🚀 开始翻译';
    }
...
async function handleDownloadTranslatedPdf() {
    if (!translatedPdfPath) {
        showToast('没有可下载的翻译 PDF', 'warning');
        return;
    }
    ...
    try {
        ...
        if (savePath) {
            showToast('翻译后 PDF 已保存到: ' + savePath, 'success');
        } else {
...
async function handleArxivSearch() {
    ...
    if (!arxivId) {
        showToast('请输入 arXiv ID', 'warning');
        return;
    }

    // Disable button during search
    btnArxivSearch.disabled = true;
    btnArxivSearch.textContent = '搜索中...';

    try {
        showToast('正在 GitHub 搜索已翻译论文...', 'info');
        ...
        if (!searchResult || !searchResult.found) {
            ...
            showToast(`未找到 ${arxivId} 的翻译，正在验证 arXiv ID...`, 'info');
            ...
            try {
                ...
                if (metadata && metadata.title) {
                    ...
                    showToast(`"${arxivId}" 不是有效的 arXiv ID`, 'warning');
                }
            } catch (validationError) {
                console.error('arXiv validation error:', validationError);
                showToast(`"${arxivId}" 不是有效的 arXiv ID 或无法访问 arXiv`, 'warning');
            }
            return;
        }
        ...
        btnArxivSearch.innerHTML = '<span class="search-icon">🔍</span>搜索';
    }
}
...
function showArxivSearchResultDialog(arxivId, searchResult) {
    ...
    // Show success toast
    showToast(`找到 ${arxivId} 的翻译！`, 'success', 3000);
}
...
async function confirmDownload() {
    ...
    try {
        ...
        for (const file of selectedFiles) {
            try {
                downloadProgressText.textContent = `正在下载 ${file.label}...`;
                ...
                showToast(`下载 ${file.label} 失败: ${error.message || error}`, 'error');
            }
        }

        if (downloadedCount > 0) {
            showToast(`成功下载 ${downloadedCount} 个文件`, 'success', 5000);
...
async function downloadGitHubTranslation(arxivId, availableFiles, searchResult) {
    try {
        ...
        for (const file of availableFiles) {
            try {
                showToast(`正在下载 ${file.label}...`, 'info');
                const savedPath = await DownloadGitHubTranslation(arxivId, file.type, saveDir);
        ...
        if (downloadedCount > 0) {
            showToast(`成功下载 ${downloadedCount} 个文件到 ${saveDir}`, 'success', 5000);
...
async function openBrowseLibrary() {
    ...
    try {
        ...
        showToast('加载共享库失败: ' + (error.message || error), 'error');
        libraryLoading.style.display = 'none';
        libraryEmpty.style.display = 'block';
    }
}
...
function renderLibraryPaperItem(paper) {
    ...
    if (paper.has_chinese) {
        ...
        badge.innerHTML = '<span>📄</span> 中文PDF';
    ...
    if (paper.has_bilingual) {
        const badge = document.createElement('span');
        badge.className = 'library-file-badge';
        badge.innerHTML = '<span>📑</span> 双语PDF';
        filesDiv.appendChild(badge);
    }
    ...
    if (paper.has_latex) {
        const badge = document.createElement('span');
        badge.className = 'library-file-badge';
        badge.innerHTML = '<span>📦</span> LaTeX';
        filesDiv.appendChild(badge);
    }
...
async function handleLibraryPaperClick(paper) {
    ...
    try {
        showToast(`正在加载 ${paper.arxiv_id} 的详细信息...`, 'info');
        ...
        if (searchResult && searchResult.found) {
            ...
            showToast(`未找到 ${paper.arxiv_id} 的翻译`, 'warning');
        }
    } catch (error) {
...
async function downloadAndOpenPaper(arxivId, searchResult) {
    try {
        ...
        updateStatus('downloading', 0, '正在下载论文...');
        
        // Determine which files to download (prefer bilingual, then chinese)
        const filesToDownload = [];
        
        if (searchResult.bilingual_pdf) {
            filesToDownload.push({ type: 'bilingual', label: '双语对照 PDF' });
        }
        if (searchResult.chinese_pdf) {
            filesToDownload.push({ type: 'chinese', label: '中文 PDF' });
        }
        
        if (filesToDownload.length === 0) {
            showToast('没有可用的 PDF 文件', 'warning');
            updateStatus('idle', 0, '');
            return;
        }
